...
GAFFER_ROOT = os.environ.get(
	'GAFFER_ROOT',
	'/home/lemans/Devel/src/gafferDependencies/gaffer-0.25.1.0-linux'
)
	...
	def _discoverApplications(self):
		...
		if sys.platform == 'linux2':
			...
			applications.extend(self._searchFilesystem(
				...
				icon='http://blog.efestolab.uk/icons/gaffer.png'
			))
...
class ApplicationLauncher(ftrack_connect.application.ApplicationLauncher):
	...
	def _getApplicationEnvironment(
		...
		gaffer_python_path = os.path.join(EFESTO_FTRACK_CONNECT_GAFFER_PATH, 'source', 'python')
		environment = ftrack_connect.application.appendPath(
			gaffer_python_path, 'PYTHONPATH', environment
		)

		gaffer_connect_path = os.path.join(EFESTO_FTRACK_CONNECT_GAFFER_PATH, 'source')
		environment = ftrack_connect.application.appendPath(
			gaffer_connect_path, 'PYTHONPATH', environment
		)

		gaffer_startup_path = os.path.join(EFESTO_FTRACK_CONNECT_GAFFER_PATH, 'resource', 'startup')
		environment = ftrack_connect.application.appendPath(
